...
TEST_BUCKET = "dp-lab-cicd/seqc"  # None
...
class TestSEQCLocal(unittest.TestCase):
    ...
    def test_local(self, platform="in_drop_v2"):
        ...
        with open("seqc_log.txt", "w") as f:
            f.write(
                "Dummy log. nose2 captures input, so no log is produced. This causes pipeline errors.\n"
            )
        test_name = "test_no_aws_%s" % platform
        makedirs(LOCAL_OUTPUT % test_name)
        if self.email is None:
            self.email = input(
                "please provide an email address for SEQC to mail results: "
            )
...
class TestReadArrayCreation(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # S3.download(BAMFILE % platform, cls.bamfile, recursive=False)
    ...
    def test_read_array_creation(self):
        ra = ReadArray.from_alignment_file(
            self.bamfile, self.translator, required_poly_t=0
        )
